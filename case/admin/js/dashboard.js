            ...
            $.each(data, function(i, v){
                $('#tbProducts tbody').append(
                    "<tr>"+
                        ...
                        "<td>" + v.size + "</td>"+
                        "<td>"+
                            "<a href='javascript:;' onclick='dashboard.productDetail(" + v.id + ");' title='edit product'><i class='fa fa-edit'></i></a> "+
                            "<a href='javascript:;' title='remove product' onclick='dashboard.remove("+ v.id +");'><i class='fa fa-trash'></i></a>"+
                        "</td>"+
                    "</tr>"
                );
            })
...
dashboard.save = function(){
    if($('#formAddEditProduct').valid()){
        if($('#productId').val() == 0){ //Add new product
            ...
            addObj.manufactory = $('#Manufactory').val();
